...
HRESULT CEffectLoader::ExecuteConstantAssignment(SBinaryConstant *pConstant, void *pLHS, D3D_SHADER_VARIABLE_TYPE lhsType)
{
    HRESULT hr = S_OK;
...
HRESULT CEffectLoader::LoadTechnique( STechnique* pTech )
{
	...
	DPF(0, "Information: technique name = %s", pTech->pName);
...
HRESULT CEffectLoader::BuildShaderBlock(SShaderBlock *pShaderBlock)
{
	...
	// NOTE: The following code requires D3DCompiler_xx.dll, and it is not available in Windows Store App for Windows 8.0.
	// If you would like to use full reflection API of effect framework in your Desktop applications,
	// please define ENABLES_D3DCOMPILER_RUNTIME_FOR_FX11_FULL_REFLECTION when building Effects11.
	// And please note that ENABLES_D3DCOMPILER_RUNTIME_FOR_FX11_FULL_REFLECTION requires D3DCompiler_xx.dll on target computers (system folder or private folder with exe).
#ifdef WINAPI_FAMILY_PARTITION
	// WinSDK 8.0 or later.
#if WINAPI_FAMILY_PARTITION(WINAPI_PARTITION_DESKTOP)
	// Desktop
#define D3DCOMPILER_RUNTIME_IS_AVAILABLE
#else
	// WinRT
#if (_MSC_VER >= 1800)
	// VC++ 2013 (WinSDK 8.1) or later.
#define D3DCOMPILER_RUNTIME_IS_AVAILABLE
#endif
#endif
#else
	// WinSDK 7.1 or earlier.
#define D3DCOMPILER_RUNTIME_IS_AVAILABLE
#endif

#if defined(D3DCOMPILER_RUNTIME_IS_AVAILABLE) && defined(ENABLES_D3DCOMPILER_RUNTIME_FOR_FX11_FULL_REFLECTION)
...
#else
#pragma message("INFO: Now Effects11 will be built as a subset (Compact Effects11). Compact Effects11 NO LONGER depends on D3DCompiler runtime, but some of the reflection methods will be restricted in compensation for the independence.")
#endif
